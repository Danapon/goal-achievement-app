...
<div class="set_block">
  ...
  <div class="set_inner">
    <h3 class="set_title">目標達成のためにやること&emsp;その{{ $subgoal_array['subgoal'] }}</h3>
  </div>
  ...
  @csrf
    ...
    @if (!$subgoal_array['isSubgoalMax'])
    <!-- サブ目標設定数が最大となれば非表示にする(true) -->
    <button type="submit" class="nes-btn next_btn" name="btn" value=1>サブ目標（その{{ $subgoal_array['subgoal'] + 1 }}）設定</button>
    @endif
