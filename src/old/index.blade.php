...
@push('scripts')
     ...
     <script>
      ...
      const current_exp = @json($current_exp);
      const next_exp = @json($next_exp); 
     </script>
...
<div class="avator">
  ...
  <img src="{{ asset($level_avator->avator_url) }}" alt="アバター" class="avator_image">
  <!-- レベルの数字の部分はlevelカラムから取得する -->
  <p class="level">LV. {{ $level_avator->level }}</p>
  <!-- レベルアップゲージ、達成ボタンが押されたタイミングでjsで動かす(valueの値を操作する) -->
  <div id="splash_text" ></div>

  @if ($next_exp)
  <p class="next_level">次のレベルまであと {{ $next_exp }} 必要です</p>
  @else
...
@if(!$goal_check && $goal_status === 0)
<div class="nes-container is-dark with-title is-centered goal">

  <p class="title section_titile">目標</p>
  
  <p class="goal_title">{{ $goals->title }}</p>
  <p class="goal_detail">{{ $goals->detail }}</p>

  <!-- サブ目標セクション -->
  <div class="subgoal">

    @foreach($goals->subgoals as $subgoal)
  ...
  <form action="{{ route('goals.update', ['goal' => $goals->id]) }}" class="goal_form" method="post">  
    @csrf
    @method('put')
